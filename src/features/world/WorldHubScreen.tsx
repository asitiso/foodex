...
export function WorldHubScreen({ progress, gameLoop }: { progress: WorldProgress; gameLoop?: GameLoopProgress }) {
  return (
    <section className="world-hub-screen" aria-label="음식 세계">
      ...
      {gameLoop && <>
        <section className="world-section game-loop-panel"><div className="section-title-row"><h2>오늘의 원정</h2><span>{gameLoop.expedition.title}</span></div><p className="world-next-action">다음 행동 · {gameLoop.expedition.nextAction}</p><div className="expedition-steps">{gameLoop.expedition.steps.map((step) => <span className={step.completed ? 'expedition-step completed' : 'expedition-step'} key={step.id}>{step.completed ? '✓' : '○'} {step.title}</span>)}</div></section>
        <section className="world-section"><div className="section-title-row"><h2>주간 축제</h2><span>{gameLoop.weeklyEvent.progress}/{gameLoop.weeklyEvent.total}</span></div><strong>{gameLoop.weeklyEvent.title}</strong><small>{gameLoop.weeklyEvent.description} · 보상: {gameLoop.weeklyEvent.reward}</small></section>
        <section className="world-section"><div className="section-title-row"><h2>주민 파티</h2><span>최대 3명</span></div><p className="world-next-action">{gameLoop.party.members.length > 0 ? gameLoop.party.members.join(' · ') : '주민을 만나면 파티가 열려요'}</p><small>{gameLoop.party.bonus}</small></section>
        {gameLoop.combinationMissions.some((mission) => !mission.completed) && <section className="world-section"><div className="section-title-row"><h2>조합 미션</h2><span>{gameLoop.combinationMissions.filter((mission) => mission.completed).length}/{gameLoop.combinationMissions.length}</span></div><div className="world-museum-list">{gameLoop.combinationMissions.map((mission) => <div className={mission.completed ? 'world-museum-room completed' : 'world-museum-room'} key={mission.id}><strong>{mission.title}</strong><span>{mission.completed ? '완료' : `${mission.foodTypes.join(' + ')}`}</span></div>)}</div></section>}
      </>}
